...
import { findActiveEmailBan, findActiveIpBan, normalizeEmail, normalizeIp } from "@/lib/bans";
...
async function ensureEmailBan(email: string, playerId: string, actorPlayerId: string | null): Promise<void> {
  ...
  if (
    ...
    if (banInsertFallback.error) {
      if (banInsertFallback.error.message.includes("bans")) throw new Error("missing_bans_schema");
      throw new Error(banInsertFallback.error.message);
    }
  } else if (banInsertPrimary.error) {
    if (banInsertPrimary.error.message.includes("bans")) throw new Error("missing_bans_schema");
    throw new Error(banInsertPrimary.error.message);
  }
}

async function ensureIpBan(ipAddress: string, reason: string | null, playerId: string, actorPlayerId: string | null): Promise<void> {
  const activeBan = await findActiveIpBan(ipAddress);
  if (activeBan) return;

  const banInsertPrimary = await supabaseServer.from("ip_bans").insert({
  ...
  if (
    ...
    const banInsertFallback = await supabaseServer.from("ip_bans").insert({
      ip_address: ipAddress,
      reason,
    });
    if (banInsertFallback.error) {
      if (banInsertFallback.error.message.includes("ip_bans")) throw new Error("missing_ip_bans_schema");
      throw new Error(banInsertFallback.error.message);
    }
  } else if (banInsertPrimary.error) {
    if (banInsertPrimary.error.message.includes("ip_bans")) throw new Error("missing_ip_bans_schema");
    throw new Error(banInsertPrimary.error.message);
  }
}
...
export async function POST(
  ...
  const ipLimit = rateLimit({ key: `admin-player-ban-ip:ip:${ip}`, limit: 30, windowMs: 60 * 60 * 1000 });
  ...
  const userLimit = rateLimit({ key: `admin-player-ban-ip:user:${admin.ctx.userId}`, limit: 20, windowMs: 60 * 60 * 1000 });
  if (!userLimit.ok) return rateLimitResponse(userLimit);

  const body = await req.json().catch(() => null);
  const reason = normalizeReason(body?.reason);
  const ipAddress = normalizeIp(body?.ipAddress);
  if (!ipAddress) {
  ...
  try {
    await ensureEmailBan(email, playerId, admin.ctx.playerId);
    await ensureIpBan(ipAddress, reason, playerId, admin.ctx.playerId);
  } catch (err) {
    const message = err instanceof Error ? err.message : "ban_write_failed";
  ...
  await writeAuditLog({
    ...
    action: "player_ban_ip",
    targetType: "player",
    targetId: playerId,
    metadata: { name: player.name, bannedEmail: email, bannedIp: ipAddress, reason },
  });

  return NextResponse.json({ banned: true, playerId, email, ipAddress });
}
